...
import { createProfile, supabase, uploadProfilePhoto } from "@/lib/integrations";
...
export default function AuthPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!supabase) return setError("La connexion n'est pas encore activée.");
    ...
    try {
      if (mode === "signup") {
        ...
        const { data, error: signUpError } = await supabase.auth.signUp({ email: email.trim(), password, options: { data: metadata } });
        ...
        if (data.user && data.session) {
          ...
          await createProfile({
            ...
            bio: role === "developer" ? specialty.trim() || "Développeur sur DevConnect" : company.trim() || "Client sur DevConnect",
